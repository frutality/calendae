                                                                    ...
                                                                    const QHash<QString, Calendar> &calendarsById)
{
    ...
    for (const Event &event : events) {
        ...
        // Repeat the item on every day the event spans so multi-day events
        // stay visible on each covered day, not just their start date.
        // lastInclusiveLocalDate() handles the all-day exclusive-endDate
        // adjustment and the timed end-at-midnight case, and never returns a
        // date before startDate. Capped defensively against pathological data.
        const QDate lastDateInclusive = event.lastInclusiveLocalDate();
