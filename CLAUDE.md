...
**Auth flow** (`AuthManager`, the sole orchestrator): `SignedOut` → `Restoring` (silent restore from a stored refresh token at startup) or `SigningIn` (interactive RFC 8252 loopback flow: opens the system browser, `OAuthLoopbackServer` catches the redirect on `127.0.0.1`) → `SignedIn`. OAuth client id/secret resolution goes through `CredentialsProvider`'s fallback chain: config file → QtKeychain → first-run dialog (`OAuthCredentialsDialog`), saved back to the keychain afterward. `AuthManager` never calls the Calendar API itself; `GoogleCalendarApi` reads the bearer token from it on every request instead of caching tokens on its own. Proactive refresh 60s before expiry normally keeps it valid, but that is a monotonic `QTimer` and does not advance during suspend, so after a long sleep the held token is already expired. A 401 therefore goes through `GoogleCalendarApi::sendAuthorized()` → `AuthManager::refreshAfterRejection()`: one shared refresh, one re-send per request. Only `invalid_grant` on that refresh means the session is dead (sign out); an unreachable/erroring token endpoint keeps the refresh token, and a token minted by a forced refresh that is refused again within 60s is reported instead of refreshed again (no loops).
